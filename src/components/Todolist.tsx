...
function Todolist(props: TodolistPropsType) {
    ...
    const classButtonActive = props.filter === "Active" ? "contained" : "outlined";
    const classButtonCompleted = props.filter === "Completed" ? "contained" : "outlined";
    ...
    return (
        <div>
                ...
                {props.tasks.map((task) => {
                    ...
                    return (
                        <div key={task.id} className={task.isDone ? "isDone" : ""}>
                            <Checkbox checked={task.isDone} onChange={(event) => onChangeTaskStatusHandler(event)}/>
                            {/*Якшо виносмо за return*/}
                            ...
                            <IconButton color="primary" onClick={() => props.removeTask(task.id, props.id)}>
                                <DeleteIcon style={{color: "darkred"}}/>
                            </IconButton>
                        </div>
                    );
            ...
            <ButtonGroup variant="outlined" aria-label="outlined button group">
                <Button variant={classButtonAll} onClick={onAllClickHandler} color={'primary'}>All</Button>
                <Button variant={classButtonActive} onClick={onActiveClickHandler} color="warning">Active</Button>
                <Button variant={classButtonCompleted} onClick={onCompletedClickHandler}
